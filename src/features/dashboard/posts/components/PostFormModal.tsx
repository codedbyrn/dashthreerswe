...
import { Category, CreatePostInput } from "@/types/postType";

export interface PostFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  postId?: string | null; // Null/undefined means Create mode, otherwise Edit mode
...
export const PostFormModal: React.FC<PostFormModalProps> = ({
  ...
  // Load Categories & Post (if editing)
  useEffect(() => {
    if (!isOpen) return;
    ...
    async function loadInitialData() {
      ...
      // If Edit Mode, fetch the specific post
      if (isEditMode && postId) {
        const resObj = await getPostById(postId);
        if (resObj.error) {
          setErrorMessage(`Failed to load post: ${resObj.error}`);
        } else if (resObj.post) {
          const p = resObj.post;
          setTitle(p.title);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const formData: CreatePostInput = {
      title,
    ...
    if (result.error) {
      if (
        result.error.toLowerCase().includes("title") ||
        ...
        if (result.error.toLowerCase().includes("status")) errors.status = "Status is required.";
        if (result.error.toLowerCase().includes("category")) errors.categoryId = "Category is required.";
      ...
      setIsSaving(false);
      onSaveSuccess(isEditMode ? "Post updated successfully!" : "Post created successfully!");
      onClose();
    }
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-default shadow-xl border border-gray-100 p-6 md:p-8 relative z-10 animate-scale-up font-sans max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
          <h2 className="font-serif text-xl font-bold text-brand-dark flex items-center gap-2">
            ...
            <span>{isEditMode ? "Edit Post Entry" : "Curate New Post"}</span>
          </h2>
          <button
            onClick={onClose}
            disabled={isSaving}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <select
                  required
                  disabled={isSaving}
                  className={`w-full bg-white border border-gray-200 hover:border-gray-300 focus:border-brand-main2 rounded-default p-3 pr-10 text-xs font-semibold text-brand-dark appearance-none focus:outline-none focus:ring-2 focus:ring-brand-main2/15 ${fieldErrors.status ? "border-red-400" : ""
                    }`}
                  value={status}
              ...
              {fieldErrors.status && (
                <span className="text-[10px] text-red-500 font-semibold">{fieldErrors.status}</span>
              )}
            </div>
