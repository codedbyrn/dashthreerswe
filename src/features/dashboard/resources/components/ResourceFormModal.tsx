...
import { Category, CreateResourceInput } from "@/types/resourceType";

export interface ResourceFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  resourceId?: string | null; // Null/undefined means Create mode, otherwise Edit mode
...
export const ResourceFormModal: React.FC<ResourceFormModalProps> = ({
  ...
  // Load Categories & Resource (if editing)
  useEffect(() => {
    if (!isOpen) return;
    ...
    async function loadInitialData() {
      ...
      // If Edit Mode, fetch the specific resource
      if (isEditMode && resourceId) {
        const resObj = await getResourceById(resourceId);
        if (resObj.error) {
          setErrorMessage(`Failed to load resource: ${resObj.error}`);
        } else if (resObj.resource) {
          const r = resObj.resource;
          setTitle(r.title);
          setUrl(r.url);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.error) {
      ...
      setIsSaving(false);
      onSaveSuccess(isEditMode ? "Resource updated successfully!" : "Resource created successfully!");
      onClose();
    }
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-default shadow-xl border border-gray-100 p-6 md:p-8 relative z-10 animate-scale-up font-sans max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
          <h2 className="font-serif text-xl font-bold text-brand-dark flex items-center gap-2">
            ...
            <span>{isEditMode ? "Edit Resource Curation" : "Curate New Resource"}</span>
          </h2>
          <button
            onClick={onClose}
            disabled={isSaving}
