...
import { Category, CreateInspirationInput } from "@/types/inspirationType";
import { ImageUpload } from "./ImageUpload";

export interface InspirationFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  inspirationId?: string | null; // Null/undefined means Create mode, otherwise Edit mode
...
export const InspirationFormModal: React.FC<InspirationFormModalProps> = ({
  ...
  // Load Categories & Inspiration (if editing)
  useEffect(() => {
    if (!isOpen) return;
    ...
    async function loadInitialData() {
      ...
      // If Edit Mode, fetch the specific inspiration
      if (isEditMode && inspirationId) {
        const resObj = await getInspirationById(inspirationId);
        if (resObj.error) {
          setErrorMessage(`Failed to load inspiration: ${resObj.error}`);
        } else if (resObj.inspiration) {
          const r = resObj.inspiration;
          setImg(r.img || "");
          setUrl(r.url || "");
    ...
    loadInitialData();
  }, [isOpen, isEditMode, inspirationId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const formData: CreateInspirationInput = {
      img: img || null,
      url: url || null,
    ...
    let result;
    if (isEditMode && inspirationId) {
      result = await updateInspiration(inspirationId, formData);
    } else {
      result = await createInspiration(formData);
    }

    if (result.error) {
      // Check if it's a validation summary or generic db error
      if (result.error.toLowerCase().includes("img") || result.error.toLowerCase().includes("url") || result.error.toLowerCase().includes("category")) {
        const errors: Record<string, string> = {};
        if (result.error.toLowerCase().includes("img")) errors.img = "Valid image URL is required if provided.";
        if (result.error.toLowerCase().includes("url")) errors.url = "Valid link URL is required if provided.";
      ...
      setIsSaving(false);
      onSaveSuccess(isEditMode ? "Inspiration updated successfully!" : "Inspiration created successfully!");
      onClose();
    }
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-default shadow-xl border border-gray-100 p-6 md:p-8 relative z-10 animate-scale-up font-sans max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center border-b border-gray-100 pb-4 mb-5">
          <h2 className="font-serif text-xl font-bold text-brand-dark flex items-center gap-2">
            ...
            <span>{isEditMode ? "Edit Inspiration Entry" : "Curate New Inspiration"}</span>
          </h2>
          <button
            onClick={onClose}
            disabled={isSaving}
        ...
        {isFetchingData ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              {fieldErrors.img && (
                <span className="text-[10px] text-red-500 font-semibold">{fieldErrors.img}</span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="Write a brief overview of the design inspiration..."
                value={description}
